...
fn main() {
    ...
    loop {
        ...
        let combination_seen = seen_combinations.iter().position(|ref r| **r == banks);
        if combination_seen.is_some() {
            println!("Iterations: {}", iteration);
            let previous_iteration = combination_seen.unwrap();
            println!("Length of loop: {}", iteration - previous_iteration);
            break;
        }
